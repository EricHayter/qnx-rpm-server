...
namespace qnx
{
    ...
    std::optional<pid_t> getParentPid(pid_t pid)
    {
        ...
        std::filesystem::path status_path = proc_base / std::to_string(pid) / "status";
        ...
        if (status.read(reinterpret_cast<char *>(&pstatus), sizeof(pstatus)) && status.good())
        {
    ...
    std::string getCommandLine(pid_t pid)
    {
        ...
        std::filesystem::path cmd_path = std::filesystem::path("/proc") / std::to_string(pid) / "cmdline";
    ...
    std::optional<std::string> getWorkingDirectory(pid_t pid)
    {
        ...
        std::filesystem::path cwd_path = std::filesystem::path("/proc") / std::to_string(pid) / "cwd";
    ...
    std::optional<BasicProcessInfo> getProcessInfo(pid_t pid)
    {
        ...
        try
        {
            ...
            {
                procfs_status pstatus;
                if (status_file.read(reinterpret_cast<char *>(&pstatus), sizeof(pstatus)) && status_file.good())
                {
