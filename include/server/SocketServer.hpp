 ...
 * @brief Socket server for handling network communications in the QNX Remote
 * Process Monitor
 *
...
public:
  ...
  using MessageHandler = std::function<std::string(
      int /* client_socket */, const std::string & /* message */)>;
  ...
  inline static const std::string GET_SIMPLE_DETAILS =
      "GetSimpleProcessDetails";
  inline static const std::string GET_DETAILED_DETAILS =
...
private:
  ...
  std::mutex
      clients_mutex_; ///< Mutex to protect concurrent access to the client list
  std::atomic<bool> running_{
      false};                 ///< Flag indicating if the server is running
  std::thread server_thread_; ///< Thread that runs the server loop
  MessageHandler
      message_handler_; ///< Callback function for processing messages
